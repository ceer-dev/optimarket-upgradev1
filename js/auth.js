 ...
 * Works with both file:// (local) and HTTP (Netlify) environments.
 */

const STORAGE_KEY = "opticaUser";

// Detect base path: if running from /views/, go up one level for login
// Standardized for Windows local file:// and Netlify HTTP
...
const Auth = {
  ...
  login: function (name, phone, nit) {
    ...
    if (window.Analytics) Analytics.trackLogin(user);
